/**
 * get_malaysian_implementations — Find Malaysian statutes that reference a specific EU directive/regulation.
 */

import type Database from '@ansvar/mcp-sqlite';
import { generateResponseMetadata, type ToolResponse } from '../utils/metadata.js';

export interface GetMalaysianImplementationsInput {
  eu_document_id: string;
  primary_only?: boolean;
  in_force_only?: boolean;
}

export interface MalaysianImplementationResult {
  document_id: string;
...
export async function getMalaysianImplementations(
  db: InstanceType<typeof Database>,
  input: GetMalaysianImplementationsInput,
): Promise<ToolResponse<MalaysianImplementationResult[]>> {
  try {
  ...
  const rows = db.prepare(sql).all(...params) as MalaysianImplementationResult[];
