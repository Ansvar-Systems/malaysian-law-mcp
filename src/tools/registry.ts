/**
 * Tool registry for Malaysian Law MCP Server.
...
import { getMalaysianImplementations, type GetMalaysianImplementationsInput } from './get-malaysian-implementations.js';
...
const LIST_SOURCES_TOOL: Tool = {
  ...
  description:
    ...
    'including the Federal Register of Legislation (Malaysian Government, Office of Parliamentary Counsel). ' +
    ...
    'Call this FIRST when you need to understand what Malaysian legal data this server covers.',
  inputSchema: { type: 'object', properties: {} },
};
...
export const TOOLS: Tool[] = [
  {
    ...
    description:
      'Search Malaysian statutes and regulations by keyword using full-text search (FTS5 with BM25 ranking). ' +
  ...
  },
  {
    name: 'get_provision',
    description:
      'Retrieve the full text of a specific provision (section) from an Malaysian statute. ' +
  ...
  },
  {
    ...
    description:
      'Validate an Malaysian legal citation against the database — zero-hallucination check. ' +
  ...
  },
  {
    ...
    description:
      'Build a comprehensive set of citations for a legal question by searching across all Malaysian statutes simultaneously. ' +
      'Returns aggregated results from multiple relevant provisions, useful for legal research on a topic. ' +
      'Use this for broad legal questions like "What are the penalties for data breaches in Malaysia?" ' +
  ...
  },
  {
    ...
    description:
      'Format an Malaysian legal citation per standard conventions. ' +
  ...
  },
  {
    name: 'check_currency',
    description:
      'Check whether an Malaysian statute or provision is currently in force, amended, repealed, or not yet in force. ' +
  ...
  },
  {
    name: 'get_eu_basis',
    description:
      'Get the EU legal basis that an Malaysian statute references or aligns with. ' +
      'Malaysia is not an EU member but Malaysian laws may reference EU directives/regulations ' +
    ...
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Malaysian statute identifier.' },
        include_articles: {
          type: 'boolean',
  ...
  {
    name: 'get_malaysian_implementations',
    description:
      'Find all Malaysian statutes that reference or align with a specific EU directive or regulation. ' +
      'Given an EU document ID (e.g., "regulation:2016/679" for GDPR), returns matching Malaysian statutes. ' +
      'Note: Malaysia does not transpose EU law but may reference or align with EU frameworks voluntarily.',
    inputSchema: {
      type: 'object',
      properties: {
        eu_document_id: {
  ...
  },
  {
    ...
    description:
      'Search for EU directives and regulations that are referenced by Malaysian legislation. ' +
    ...
    inputSchema: {
      ...
      properties: {
        ...
        has_malaysian_implementation: {
          type: 'boolean',
          description: 'If true, only return EU documents referenced by Malaysian legislation.',
        },
  ...
  },
  {
    ...
    description:
      'Get the EU legal basis for a SPECIFIC provision within an Malaysian statute. ' +
  ...
  },
  {
    ...
    description:
      'Check EU alignment status for an Malaysian statute or provision. ' +
      ...
      'Note: Malaysia is not bound by EU law; this checks voluntary alignment and cross-references.',
    inputSchema: {
      type: 'object',
      properties: {
        document_id: { type: 'string', description: 'Malaysian statute identifier.' },
    ...
    try {
      ...
      switch (name) {
        ...
        case 'get_eu_basis':
          ...
          break;
        case 'get_malaysian_implementations':
          result = await getMalaysianImplementations(db, args as unknown as GetMalaysianImplementationsInput);
          break;
