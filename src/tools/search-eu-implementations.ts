/**
 * search_eu_implementations — Search EU directives/regulations referenced by Malaysian legislation.
 */
...
export interface SearchEUImplementationsInput {
  ...
  year_from?: number;
  year_to?: number;
  has_malaysian_implementation?: boolean;
  limit?: number;
}
...
export interface EUImplementationSearchResult {
  ...
  malaysian_statute_count: number;
}
...
export async function searchEUImplementations(
  ...
  let sql = `
    SELECT
      ...
      ed.title,
      ed.short_name,
      COUNT(DISTINCT er.document_id) as malaysian_statute_count
  ...
  if (input.has_malaysian_implementation) {
    sql += ' HAVING malaysian_statute_count > 0';
  }
